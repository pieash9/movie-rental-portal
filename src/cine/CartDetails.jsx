...
const CartDetails = ({ onClose }) => {
  ...
  const handleDeleteCart = (e, item) => {
    ...
    toast.success(`The movie "${item.title}" removed from cart successfully!`);
  };
  return (
        ...
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          <div className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14">
            {state.cartData.length === 0 ? (
              <p className=" text-3xl text-[#9fa0a4] dark:text-[#575A6E] font-semibold">
                Cart is empty!
              </p>
            ) : (
              state.cartData.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4">
                  <div className="flex items-center gap-4">
                    <img
                      className="rounded overflow-hidden size-[50px]"
